...
export default function Voronoi<TDatum>() {
    ...
    const { onFocusDatum, onClickDatum, tooltip, primaryCursor, secondaryCursor, showVoronoi, interactionMode } =
        getOptions();
    ...
    const needsVoronoi =
        isInteracting && (showVoronoi || onFocusDatum || onClickDatum || tooltip || primaryCursor || secondaryCursor);
...
function PrimaryVoronoi<TDatum>({ handleFocus }: { handleFocus: (datum: Datum<TDatum> | null) => void }) {
    ...
    return React.useMemo(() => {
        ...
        const columns = preColumns.map((datums, i, all) => {
            ...
            if (prev) {
                const secondaryAxis = secondaryAxes.find(d => d.id === prev[0].secondaryAxisId)!;
                const prevPx = getPrimary(prev[0], primaryAxis, secondaryAxis, useBarPx);
                primaryStart = primaryPx - (primaryPx - prevPx) / 2;
            }

            if (next) {
                const secondaryAxis = secondaryAxes.find(d => d.id === next[0].secondaryAxisId)!;
            ...
            return {
                ...
                datumBoundaries: datums
                    ...
                    .map((datum, i, all) => {
                        ...
                        const value =
                            secondaryAxis?.scale(secondaryAxis.stacked ? datum.stackData?.[1] : datum.secondaryValue) ??
                            NaN;
                        ...
                        if (prev) {
                            const prevAxis = secondaryAxes.find(d => d.id === prev?.secondaryAxisId);
                            const prevValue =
                                prevAxis?.scale(prevAxis.stacked ? prev.stackData?.[1] : prev.secondaryValue) ?? NaN;
                            secondaryStart = value - (value - prevValue) / 2;
                        }

                        if (next) {
                            const nextAxis = secondaryAxes.find(d => d.id === next?.secondaryAxisId);
                            const nextValue =
                                nextAxis?.scale(nextAxis.stacked ? next.stackData?.[1] : next.secondaryValue) ?? NaN;
                    ...
                    return (
                        <g key={`${column.primaryPx}_${i}`}>
                            {column.datumBoundaries.map((datumBoundary, i) => {
                                const x1 = !primaryAxis.isVertical ? column.primaryStart : datumBoundary.secondaryStart;
                                const x2 = !primaryAxis.isVertical ? column.primaryEnd : datumBoundary.secondaryEnd;
                                const y1 = !primaryAxis.isVertical ? datumBoundary.secondaryStart : column.primaryStart;
                                const y2 = !primaryAxis.isVertical ? datumBoundary.secondaryEnd : column.primaryEnd;
...
function SingleVoronoi<TDatum>({ handleFocus }: { handleFocus: (datum: Datum<TDatum> | null) => void }) {
    const {
        primaryAxis,
        series,
        secondaryAxes,
    ...
    const voronoi = delaunay.voronoi([0, 0, gridDimensions.width, gridDimensions.height]);
