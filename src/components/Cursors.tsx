...
type ResolvedCursorOptions = TSTB.Object.Required<CursorOptions, "show" | "showLine" | "showLabel">;

const getLineBackgroundColor = (dark: boolean, bandWidth: number) =>
    dark ? `rgba(255,255,255,${bandWidth > 4 ? 0.15 : 0.3})` : `rgba(0, 26, 39, ${bandWidth > 4 ? 0.15 : 0.3})`;
...
export default function Cursors<TDatum>() {
    ...
    const resolvedPrimaryOptions = React.useMemo(
        () =>
            defaultCursor(
                !primaryOptions ? { show: false } : typeof primaryOptions === "boolean" ? {} : primaryOptions
            ),
        [primaryOptions]
    );
    ...
    const resolvedSecondaryOptions = React.useMemo(
        () =>
            defaultCursor(
                !secondaryOptions ? { show: false } : typeof secondaryOptions === "boolean" ? {} : secondaryOptions
            ),
        [secondaryOptions]
    );
...
function Cursor<TDatum>(props: { primary?: boolean; options: ResolvedCursorOptions }) {
    const { getOptions, svgRef, gridDimensions, focusedDatumState, primaryAxis, secondaryAxes } =
    ...
    const secondaryAxis = secondaryAxes.find(d => d.id === latestFocusedDatum?.secondaryAxisId) ?? secondaryAxes[0];
    ...
    const resolveValue = (d: Datum<TDatum> | null) =>
        d ? (axis.stacked ? d.stackData?.[1] : props.primary ? d.primaryValue : d.secondaryValue) : undefined;
    ...
    const latestPropsValue = useLatestWhen(props.options.value, props.options.value != null);

    const latestDatumValue = useLatestWhen(resolveValue(latestFocusedDatum), resolveValue(latestFocusedDatum) != null);
