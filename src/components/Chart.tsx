...
function defaultChartOptions<TDatum>(options: ChartOptions<TDatum>): RequiredChartOptions<TDatum> {
    return {
        ...options,
        initialWidth: options.initialWidth ?? 300,
        initialHeight: options.initialHeight ?? 200,
        getSeriesOrder: options.getSeriesOrder ?? ((series: Series<TDatum>[]) => series),
        ...
        intersectionObserverRootMargin: options.intersectionObserverRootMargin ?? "1000px",
...
export function Chart<TDatum>({
    ...
    const [chartElement, setContainerElement] = React.useState<HTMLDivElement | null>(null);
...
function ChartInner<TDatum>({
    ...
    const primaryAxisOptions = React.useMemo((): BuildAxisOptions<TDatum> => {
        ...
        const axisOptions = axisOptionsWithScaleType(options.primaryAxis, firstValue);

        return { position: "bottom", ...axisOptions };
    }, [options.data, options.primaryAxis]);

    const secondaryAxesOptions = React.useMemo(() => {
        return options.secondaryAxes.map((secondaryAxis, i): BuildAxisOptions<TDatum> => {
    ...
    const gridDimensions = React.useMemo((): GridDimensions => {
        const padding = {
            left: typeof options.padding === "object" ? (options.padding.left ?? defaultPadding) : options.padding,
            right: typeof options.padding === "object" ? (options.padding.right ?? defaultPadding) : options.padding,
            bottom: typeof options.padding === "object" ? (options.padding.bottom ?? defaultPadding) : options.padding,
            top: typeof options.padding === "object" ? (options.padding.top ?? defaultPadding) : options.padding,
        };
    ...
    const series = React.useMemo(() => {
        ...
        for (let seriesIndex = 0; seriesIndex < options.data.length; seriesIndex++) {
            ...
            indicesByAxisId[`${secondaryAxisId}`] = indicesByAxisId[`${secondaryAxisId}`] ?? 0;
            const seriesIndexPerAxis = indicesByAxisId[`${secondaryAxisId}`];

            indicesByAxisId[`${secondaryAxisId}`]++;

            for (let datumIndex = 0; datumIndex < originalDatums.length; datumIndex++) {
    ...
    const secondaryAxes = React.useMemo(() => {
        return secondaryAxesOptions.map(secondaryAxis => {
            return buildAxisLinear<TDatum>(false, secondaryAxis, series, allDatums, gridDimensions, width, height);
        });
    ...
    const [datumsByInteractionGroup, datumsByTooltipGroup] = React.useMemo(() => {
        ...
        let getInteractionKey = (datum: Datum<TDatum>) => `${getInteractionPrimary(datum)}`;
        let getTooltipKey = (datum: Datum<TDatum>) => `${datum.primaryValue}`;

        if (options.interactionMode === "closest") {
            getInteractionKey = datum => `${getInteractionPrimary(datum)}_${datum.secondaryValue}`;
        }
    ...
    const getDatumStatusStyle = React.useCallback(
        (datum: Datum<TDatum>, focusedDatum: Datum<TDatum> | null) => {
            ...
            const statusStyles = getOptions().getDatumStyle?.(datum as Datum<TDatum>, status) ?? {};
...
function getFirstDefinedValue<TDatum>(options: AxisOptions<TDatum>, data: UserSerie<TDatum>[]) {
...
function sortDatumsBySecondaryPx<TDatum>(datums: Datum<TDatum>[], secondaryAxes: Axis<TDatum>[]) {
    if (secondaryAxes.every(d => d.stacked)) {
        const differingInverts = secondaryAxes.some(d => d.invert) && secondaryAxes.some(d => !d.invert);
    ...
    return [...datums].sort((a, b) => {
        ...
        const aPx = aAxis?.scale(aAxis.stacked ? a.stackData?.[1] : a.secondaryValue) ?? NaN;

        const bPx = bAxis?.scale(bAxis.stacked ? b.stackData?.[1] : b.secondaryValue) ?? NaN;
