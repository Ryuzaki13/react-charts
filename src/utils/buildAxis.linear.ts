import { range as d3Range, extent, max, median, min } from "d3-array";
import { scaleBand, ScaleBand, scaleLinear, ScaleLinear, scaleLog, scaleTime, ScaleTime, scaleUtc } from "d3-scale";
...
export default function buildAxisLinear<TDatum>(
    ...
    const range: [number, number] = isVertical ? [gridDimensions.height, 0] : [0, gridDimensions.width];
...
function buildTimeAxis<TDatum>(
    ...
    series = isPrimary ? series : series.filter(s => s.secondaryAxisId === options.id);
    ...
    if (minValue === undefined || maxValue === undefined) {
        console.info("Invalid scale min/max", {
            ...
            series,
            range,
            values: allDatums.map(d => (isPrimary ? d.primaryValue : d.secondaryValue)),
        });
        isInvalid = true;
    }
    ...
    const scaleFormat = (value: Date) =>
        options.formatters?.scale?.(value, { ...formatters, scale: undefined }) ?? contextFormat(autoFormatStr, value);
...
function buildLinearAxis<TDatum>(
    ...
    const scaleFormat = (value: number) =>
        options.formatters?.scale?.(value, { ...formatters, scale: undefined }) ?? defaultFormat(value);
...
function stackSeries<TDatum>(series: Series<TDatum>[], axisOptions: AxisOptions<TDatum>) {
    const seriesIndices = Object.keys(series);
    const stacker = stack()
        .keys(seriesIndices)
        .value((_, seriesIndex, index) => {
            const originalDatum = series[Number(seriesIndex)]?.datums[index]?.originalDatum;

            const val = typeof originalDatum !== "undefined" ? axisOptions.getValue(originalDatum) : 0;
