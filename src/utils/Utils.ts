...
export function getSeriesStatus<TDatum>(series: Series<TDatum>, focusedDatum: Datum<TDatum> | null): SeriesFocusStatus {
...
export function getDatumStatus<TDatum>(datum: Datum<TDatum>, focusedDatum: Datum<TDatum> | null): DatumFocusStatus {
    ...
    if (
        datum.tooltipGroup?.some(groupDatum => {
            groupDatum.seriesId === focusedDatum?.seriesId && groupDatum.index === focusedDatum?.index;
        })
    ) {
        return "groupFocused";
    }
...
export function materializeStyles(style: SeriesStyles | DatumStyles = {}, defaults: SeriesStyles | DatumStyles = {}) {
